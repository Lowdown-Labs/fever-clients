...
pub async fn query(configuration: &configuration::Configuration, query_request: models::QueryRequest) -> Result<models::QueryResult, Error<QueryError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::QueryResult`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::QueryResult`")))),
        }
    } else {
        let content = resp.text().await?;
