...
/// struct for typed errors of method [`associate_media`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum AssociateMediaError {
...
pub async fn associate_media(configuration: &configuration::Configuration, blob_id: i32, associate_request: models::AssociateRequest, customer_id: Option<&str>) -> Result<models::MediaInfo, Error<AssociateMediaError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_blob_id = blob_id;
    let p_body_associate_request = associate_request;
    let p_query_customer_id = customer_id;

    let uri_str = format!("{}/v1/media/{blob_id}/associate", configuration.base_path, blob_id=p_path_blob_id);
    ...
    req_builder = req_builder.json(&p_body_associate_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<AssociateMediaError> = serde_json::from_str(&content).ok();
