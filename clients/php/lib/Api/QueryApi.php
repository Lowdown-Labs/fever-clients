    ...
    public function queryRequest($query_request, string $contentType = self::contentTypes['query'][0])
    {
        ...
        if ($query_request === null || (is_array($query_request) && count($query_request) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $query_request when calling query'
            );
        }
        ...
        if (isset($query_request)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                try {
                    $httpBody = json_encode(ObjectSerializer::sanitizeForSerialization($query_request), JSON_THROW_ON_ERROR);
