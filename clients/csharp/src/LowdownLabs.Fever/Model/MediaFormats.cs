/*
 * FEVER Retrieval
 *
 * The multimodal media database. Images, video, audio, office documents, and LiDAR in one table: hybrid vector + full-text search, dedupe, seek-to-moment video hits, whisper transcript spans, collection grouping, and asset-to-asset attachments. Tenancy, scale, and isolation are automatic; the tenant is taken from the API key.
 *
 * The version of the OpenAPI document: 0.4.0
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using FileParameter = LowdownLabs.Fever.Client.FileParameter;
using OpenAPIDateConverter = LowdownLabs.Fever.Client.OpenAPIDateConverter;

namespace LowdownLabs.Fever.Model
{
    /// <summary>
    /// MediaFormats
    /// </summary>
    [DataContract(Name = "MediaFormats")]
    public partial class MediaFormats : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="MediaFormats" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected MediaFormats() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="MediaFormats" /> class.
        /// </summary>
        /// <param name="audio">audio (required).</param>
        /// <param name="document">document (required).</param>
        /// <param name="image">image (required).</param>
        /// <param name="lidar">lidar (required).</param>
        /// <param name="video">video (required).</param>
        public MediaFormats(List<string> audio = default, List<string> document = default, List<string> image = default, List<string> lidar = default, List<string> video = default)
        {
            // to ensure "audio" is required (not null)
            if (audio == null)
            {
                throw new ArgumentNullException("audio is a required property for MediaFormats and cannot be null");
            }
            this.Audio = audio;
            // to ensure "document" is required (not null)
            if (document == null)
            {
                throw new ArgumentNullException("document is a required property for MediaFormats and cannot be null");
            }
            this.Document = document;
            // to ensure "image" is required (not null)
            if (image == null)
            {
                throw new ArgumentNullException("image is a required property for MediaFormats and cannot be null");
            }
            this.Image = image;
            // to ensure "lidar" is required (not null)
            if (lidar == null)
            {
                throw new ArgumentNullException("lidar is a required property for MediaFormats and cannot be null");
            }
            this.Lidar = lidar;
            // to ensure "video" is required (not null)
            if (video == null)
            {
                throw new ArgumentNullException("video is a required property for MediaFormats and cannot be null");
            }
            this.Video = video;
        }

        /// <summary>
        /// Gets or Sets Audio
        /// </summary>
        [DataMember(Name = "audio", IsRequired = true, EmitDefaultValue = true)]
        public List<string> Audio { get; set; }

        /// <summary>
        /// Gets or Sets Document
        /// </summary>
        [DataMember(Name = "document", IsRequired = true, EmitDefaultValue = true)]
        public List<string> Document { get; set; }

        /// <summary>
        /// Gets or Sets Image
        /// </summary>
        [DataMember(Name = "image", IsRequired = true, EmitDefaultValue = true)]
        public List<string> Image { get; set; }

        /// <summary>
        /// Gets or Sets Lidar
        /// </summary>
        [DataMember(Name = "lidar", IsRequired = true, EmitDefaultValue = true)]
        public List<string> Lidar { get; set; }

        /// <summary>
        /// Gets or Sets Video
        /// </summary>
        [DataMember(Name = "video", IsRequired = true, EmitDefaultValue = true)]
        public List<string> Video { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class MediaFormats {\n");
            sb.Append("  Audio: ").Append(Audio).Append("\n");
            sb.Append("  Document: ").Append(Document).Append("\n");
            sb.Append("  Image: ").Append(Image).Append("\n");
            sb.Append("  Lidar: ").Append(Lidar).Append("\n");
            sb.Append("  Video: ").Append(Video).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
